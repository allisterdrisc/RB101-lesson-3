...
# The line informal_greeting << ' there' was using the String#<< method, which
# modifies the object that called it. This means that the original object was
# changed, thereby impacting the value in greetings. If instead of modifying
# the original object, we wanted to only modify informal_greeting but not greetings,
# there are a couple of options:

# we could initialize informal_greeting with a reference to a new
# object containing the same value:
  informal_greeting = greetings[:a].clone.
# we can use string concatenation which returns a new String
...
# an input string is an IP address representing dot-separated
...
# the while loop. Once he breaks, control falls through to the return true
